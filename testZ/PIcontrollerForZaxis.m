...
fileName = 'zCoil_Epoch_2024_01_08.mat';
...
figure();
p1 = subplot(3,2,1);
    h1 = plot(z_coil_data(1,:), z_coil_data(2,:),'r.','LineWidth',4); hold on;
    xlabel('t(s)'); ylabel('X(nT)'); axis equal; grid on;
    title('Static measurements'); 
p2 = subplot(3,2,3);
    h2 = plot(z_coil_data(1,:), z_coil_data(3,:),'g.','LineWidth',4); hold on;
    xlabel('t(s)'); ylabel('Y(nT)'); axis equal; grid on;   
p3 = subplot(3,2,5);
    h3 = plot(z_coil_data(1,:), z_coil_data(4,:),'b.','LineWidth',4); hold on;
    xlabel('t(s)'); ylabel('Z(nT)'); axis equal; grid on;  
p4 = subplot(1,2,2);
    h4 = plot(z_coil_data(5,:), z_coil_data(6,:),'k.','LineWidth',4); hold on;
...
while 1
    ...
    %%%PID controller
    % Calculate error
    current_value = actualMeasures(3);
    ...
    fprintf(z_axis,['VOLTage ', num2str(V,'%.2f')]);
    if (firstLoop==1)
    ...
    if exist('DataReceived','var')
        ...
        z_coil_data = [z_coil_data,currentData];
    % %%Update graphs
